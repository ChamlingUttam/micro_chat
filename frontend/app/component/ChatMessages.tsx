...
const ChatMessages = ({selectedUser,messages,loggedInUser}: chatMessages)  => {
    ...
    const uniqueMessages = useMemo(()=>{
        ...
        return messages.filter((messages)=>{
            if(seen.has(messages._id))
            {
                return false
            }
            seen.add(messages._id)
            return true
        })
    },[messages])
  ...
  return (
    ...
    <div className='flex-1 overflow-hidden'>
        <div className='h-full max-h-[calc(100vh-215px)] overflow-y-auto p-2 space-y-2 custom-scroll'>
            {
                !selectedUser ? (
                    <p className='text-gray-400 text-center mt-20'>select a user to start chat</p>
                ) : (
                    <div>
                        {
                            uniqueMessages.map((e,i)=>{
                                ...
                                return (
                                    <div className={`flex flex-col gap-1 mt-2 ${isSentByMe ? "items-end" : "items-start"}`} key={uniqueKey}>
                                        ...
                                        <div className={`flex items-center gap-1 text-sm text-gray-400 ${isSentByMe ? "pr-2 flex-row-reverse" : "pl-2"}`}>
                                                ...
                                                <div className='flex items-center ml-1'>
                                                    {
                                                        ...
                                                        </div> ):( <Check className='w-3 h-2 text-gray-500'/>)
                                                    }
                                                </div>
                                            }
                                        </div>
