...
const MessageInput = ({selectedUser,message,handleMessageSend,setMessage}:MessageInputProps) => {
            ...
            imageFile && (
                <div className='relative w-fit'>
                    ...
                    //  height={96}
                      ...
                      <button onClick={()=>setImageFile(null) } className='absolute -top-2 -right-2 bg-black rounded-full p-1'><X className='w-4 h-4 text-white'/></button>
                </div>
            )
        }

        <div className="flex items-center gap-2">
            <label className="cursor-pointer bg-gray-700 hover:bg-gray-800 rounded-lg px-3 py-2 transaction-colors">
            ...
            <button type='submit' disabled={(!imageFile && !message)||isUploading} className='bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed text-white'>
                {
                    isUploading ? (
                        <Loader2 className='w-4 h-4 animation-spin'/> ): (<Send className='w-4 h-4'/>)
